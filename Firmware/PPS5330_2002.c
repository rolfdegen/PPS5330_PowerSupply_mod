 ...
 * MCU ATmega88PA
 *
 * Created: 24.04.2024 Version 2.002
...
//#define AD_input_T_Transformer                 (1<<ADC_AD1)               /* Input 2 */ Sensor change
//#define AD_input_T_Heatsink    ((1<<ADC_AD0) | (1<<ADC_AD1))              /* Input 3 */ Sensor change
#define AD_input_T_Transformer   ((1<<ADC_AD0) | (1<<ADC_AD1))              /* Input 2 */
#define AD_input_T_Heatsink						 (1<<ADC_AD1)				/* Input 3 */
...
#define BUTTON_PRESS_DEBOUNCE 50   /* 50ms  the standby key bounces very badly */
...
uint16_t sens1_value = 0;
uint16_t sens2_value = 0;
...
int32_t buttonLessEvent[BUTTON_COUNT] = { 0,0,0,0,0,0,0 };
...
void print_T_heatsink_result(uint16_t meas_time)
{	
    #define calib_offset_temp1 5910      // Calibration to be done
    #define calib_factor_temp1 175608    // Calibration to be done (use 196608/65536 = 3 as a starting point)
...
void print_T_transformer_result(uint16_t meas_time)
{	
	#define calib_offset_temp2 5665      // Calibration to be done
	#define calib_factor_temp2 182908    // Calibration to be done (use 196608/65536 = 3 as a starting point)
...
void Measurement(void)
{
	...
	if (Meas_phase == MEAS_PHASE_READ_RESULT)
	{
		...
		else if (counter%10 == 0)	// 25
		{
			Meas_type = MEAS_TYPE_T_HEATSINK;
		}
		else if (counter%7 == 0)	// 2
		{
...
void set_Usoll (int16_t value)
{
	//calib_counts_per_20V = 10022;
	//calib_counts_offset_0V = 123;
	//calib_counts_per_20V = 10280;
	//calib_counts_offset_0V = 126;
...
void set_Isoll (int16_t value)
{
    //calib_counts_per_1200mA = 5758;
	//calib_counts_offset_0mA = 118;
	//calib_counts_per_1200mA = 5880;
	//calib_counts_offset_0mA = 140;
